document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePageFooter(doc, pageNumber, totalPages, currentUserData) {
        ...
        const totalTime = formTiming.getFormFillingTime();
        const formattedTime = totalTime ? `${totalTime.toFixed(1)} seconds` : 'N/A';
        const pageText = `Page ${pageNumber} of ${totalPages}`;
        const infoText = `RTU Serial No: ${currentUserData.rtuSerial} | Contract No: ${currentUserData.contractNo}`;
        const generatedText = `Generated by ${currentUserData.name} in ${formattedTime} on ${new Date().toLocaleString()}`;
        const pageWidth = doc.internal.pageSize.getWidth();
        const pageHeight = doc.internal.pageSize.getHeight();
